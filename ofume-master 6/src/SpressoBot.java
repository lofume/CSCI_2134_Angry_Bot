...
public class SpressoBot extends TimBot { 
   ...
   */
  SpressoBot(int id, int jolts) {
    super( id, jolts );
    personality = 'S';
  }

  //Factory pattern replacement
  public static SpressoBot createSpressoBot(int id, int jolts) {
  ...
  public int getNextMove() {
    ...
    if( energyLevel > 0 ) {
      ...
      for( int i = 0; i < scores.length; i++ ) {
        ...
        if( ( i != District.CURRENT_DISTRICT) && botsSensed[i] ) {
          scores[i] += 2;
          adj = 1;
        }
      }
      // Only the current district will have an adjacent score
      scores[District.CURRENT_DISTRICT] += adj;
