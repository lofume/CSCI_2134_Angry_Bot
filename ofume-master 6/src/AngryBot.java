...
public class AngryBot extends SpressoBot {
   ...
   */
  AngryBot(int id, int jolts) {
    super( id, jolts );
    personality = 'A';
  }

  //Factory pattern replacement
  public static AngryBot createAngryBot(int id, int jolts) {
    return new AngryBot(id, jolts);
  }
  ...
  public int getNextMove() {
    ...
    if( energyLevel > 2 ) {
      ...
      int min = scores[District.CURRENT_DISTRICT] + 1;
      for( int i = 0; i < scores.length; i++ ) {
        if( min > scores[i] ) {
          min = scores[i];
          move = i;
      ...
      if( move != District.CURRENT_DISTRICT) {
        energyLevel--;
      }
    } else {
